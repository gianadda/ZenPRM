IF EXISTS (select * from dbo.sysobjects where id = object_id(N'ufnGetEntityProjectRequirementAnswerByIdentSubset') 
	AND OBJECTPROPERTY(id, N'IsTableFunction') = 1)
DROP FUNCTION ufnGetEntityProjectRequirementAnswerByIdentSubset
GO

/* ufnGetEntityProjectRequirementAnswerByIdentSubset
 *
 *  Gets the list of answers for a given entity Project Requirement Ident (for a given subset)
 *
 *
*/
CREATE FUNCTION ufnGetEntityProjectRequirementAnswerByIdentSubset(@tblEntity AS [EntitySearchOutput] READONLY, @bntEntityProjectIdent AS BIGINT, @bntEntityProjectRequirementIdent BIGINT)
...
AS
	...
	BEGIN
		...
		UPDATE
			tR
		SET
			EntityProjectEntityIdent = EPE.Ident
		FROM
			@tblResults tR
			INNER JOIN
			EntityProjectEntity EPE WITH (NOLOCK)
				ON EPE.EntityIdent = tR.EntityIdent
		WHERE
